...
if ( $properties_for_map_query->have_posts() ) :
	...
	<script type="text/javascript">
		function initializePropertiesMap() {
			...
			var mapOptions = {
				zoom : 12, maxZoom : 16, scrollwheel : false
			};
			...
			for( var i = 0; i < properties.length; i++ ) {
				...
				markers[i] = new google.maps.Marker( {
					...
					map : map, // icon: properties[i].icon,
					icon : image,
				...
				if( properties[i].thumb ) {
					innerHTML += '<a class="thumb-link" href="' + properties[i].url + '">' + '<img class="prop-thumb" src="' + properties[i].thumb + '" alt="' + properties[i].title + '"/>' + '</a>';
				}
